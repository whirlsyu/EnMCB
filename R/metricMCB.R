  ...
  if (!Method %in% c("svm","cox","enet")){
    ...
    for (mcb in seq_len(nrow(MCBset))) {
      ...
      if (!is.null(svm_model)) {
        ...
        write_MCB[2]<-survivalROC::survivalROC(Stime = times[,1],status = times[,2],marker = MCB_svm_matrix_training[mcb,rz],predict.time = predict_time,method = "NNE",span =0.25*length(times)^(-0.20)  )$AUC
        ...
        if (!is.null(testing_set)){
          ...
          write_MCB[3]<-survivalROC::survivalROC(Stime = Surv.new[,1],status = Surv.new[,2],marker = MCB_svm_matrix_test_set[mcb,],predict.time = predict_time,method = "NNE",span =0.25*length(Surv.new)^(-0.20) )$AUC
    ...
    for (mcb in seq_len(nrow(MCBset))) {
      ...
      if (!is.null(univ_models)) {
        ...
        write_MCB[2]<-survivalROC::survivalROC(Stime = times[,1],status = times[,2],marker = MCB_cox_matrix_training[mcb,rz],predict.time = predict_time,method = "NNE",span = 0.25*length(times)^(-0.20))$AUC
        #if it has a independent test set
        if (!is.null(testing_set)){
          MCB_cox_matrix_test_set[mcb,]<-stats::predict(univ_models, data.frame(t(testing_set[CpGs,])))
          write_MCB[3]<-survivalROC::survivalROC(Stime = Surv.new[,1],status = Surv.new[,2],marker = MCB_cox_matrix_test_set[mcb,],predict.time = predict_time,method = "NNE",span =0.25*length(Surv.new)^(-0.20))$AUC
          if (abs(write_MCB[2]+write_MCB[3]-1)>best_auc){
            best_auc<-abs(write_MCB[2]+write_MCB[3]-1)
    ...
    for (mcb in seq_len(nrow(MCBset))) {
      ...
      if (!is.null(enet_model)) {
        ...
        write_MCB[2]<-survivalROC::survivalROC(Stime = times[,1],status = times[,2],marker = MCB_enet_matrix_training[mcb,rz],predict.time = predict_time,method = "NNE",span = 0.25*length(times)^(-0.20) )$AUC
        #if it has a independent test set
        if (!is.null(testing_set)){
          # lambda.min was used.
          MCB_enet_matrix_test_set[mcb,]<-stats::predict(enet_model, t(testing_set[CpGs,]),s=lambda_min_corrected)
          write_MCB[3]<-survivalROC::survivalROC(Stime = Surv.new[,1],status = Surv.new[,2],marker = MCB_enet_matrix_test_set[mcb,],predict.time = predict_time,method = "NNE",span = 0.25*length(Surv.new)^(-0.20))$AUC
          if (abs(write_MCB[2]+write_MCB[3]-1)>best_auc){
            best_auc<-abs(write_MCB[2]+write_MCB[3]-1)
    ...
    for (mcb in seq_len(nrow(MCBset))) {
      ...
      write_MCB<-rep(NA,5)
      names(write_MCB)<-c('MCB_no','AUC_train','AUC_test','CI_train','CI_test')
      #save the mcb number
      write_MCB[1]<-as.numeric(MCBset[mcb,'MCB_no'])
      # build temp variable for saving the results.
      # MCB number
      ...
      if (!is.null(svm_model)) {
        MCB_svm_matrix_training[mcb,]<-stats::predict(svm_model, data.frame(t(training_set[CpGs,])))$predicted
        write_MCB[2]<-survivalROC::survivalROC(Stime = times[,1],status = times[,2],marker = MCB_svm_matrix_training[mcb,rz],predict.time = predict_time,method = "NNE",span =0.25*length(times)^(-0.20)  )$AUC
        write_MCB[4]<-survival::survConcordance(surv_data ~ marker,
                                                data.frame(surv_data = times,
                                                marker = MCB_svm_matrix_training[mcb,]))$concordance
        #if it has a independent test set
        if (!is.null(testing_set)){
          MCB_svm_matrix_test_set[mcb,]<-stats::predict(svm_model, data.frame(t(testing_set[CpGs,])))$predicted
          #AUC
          ...
          #CI
          write_MCB[4]<-survival::survConcordance(surv_data ~ marker,
                                                  data.frame(surv_data = Surv.new,
                                                             marker = MCB_svm_matrix_test_set[mcb,]))$concordance
          if (abs(write_MCB[2]+write_MCB[3]-1)>best_auc){
            best_auc<-abs(write_MCB[2]+write_MCB[3]-1)
            best_model<-list(mcb,svm_model)
          }
